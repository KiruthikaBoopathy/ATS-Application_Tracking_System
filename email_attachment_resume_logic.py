...
def extract_text_from_pdf(attachment_data):
    try:
        ...
        if "Candidate Resume" in text_content:
            info["Name"] = text_content.split("Candidate Resume", 1)[1].split("Job_title", 1)[0].strip("\n")
        for line in lines:
            if line.startswith("Email"):
                info["Email"] = line.split(":", 1)[1]
            if line.startswith("Mobile"):
                info["Mobile"] = line.split(":", 1)[1]
            if line.startswith("location "):
                info["location"] = line.split(":", 1)[1]
        if "Education" in text_content:
            info["Education"] = text_content.split("Education", 1)[1].split("Skills", 1)[0]
        if "Skills" in text_content:
            info["Skills"] = text_content.split("Skills", 1)[1].split('Work Experience', 1)[0]
...
def handle_python_attachment(attachment_data):
    try:
        # Python script files are plain text, so decode the content
